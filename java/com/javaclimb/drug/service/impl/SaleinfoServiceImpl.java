...
public class SaleinfoServiceImpl extends ServiceImpl<SaleinfoMapper, Saleinfo> implements ISaleinfoService {
     ...
     */
    @Override
    public IPage<Saleinfo> selectSaleinfoPage(Integer page,Integer limit,String param) {
        QueryWrapper<Saleinfo> queryWrapper = new QueryWrapper<>();
        if(StringUtils.isNotEmpty(param)){
            queryWrapper.like("dname",param);
        }
        Page<Saleinfo> page1 = new Page<>(page,limit);
        return saleinfoMapper.selectPage(page1,queryWrapper);
    }
